...
// ── Shared materials (one GPU upload, reused everywhere) ──
...
const RING_MAT = new THREE.MeshBasicMaterial({ color: "#d4a574", side: THREE.DoubleSide, transparent: true, opacity: 0.5 });
const SUN_MAT = new THREE.MeshBasicMaterial({ color: "#f59e0b" });

// Material cache for planet/moon colors
const matCache = new Map<string, THREE.MeshBasicMaterial>();
function getMat(color: string) {
...
interface PlanetProps { radius: number; orbitRadius: number; speed: number; color: string; moons?: MoonDef[]; hasRings?: boolean }
...
function Moon({ radius, orbitRadius, speed, color }: MoonDef) {
  ...
  return (
    <>
      ...
      <mesh ref={ref} geometry={MOON_GEO} material={getMat(color)} scale={radius} />
    </>
  );
}

function Planet({ radius, orbitRadius, speed, color, moons, hasRings }: PlanetProps) {
  ...
  return (
    <>
      ...
      <group ref={groupRef}>
        <mesh geometry={PLANET_GEO} material={getMat(color)} scale={radius} />
        {hasRings && <mesh geometry={RING_GEO} material={RING_MAT} scale={radius} />}
...
function SolarSystemScene() {
  ...
  return (
    <group ref={groupRef} position={[0, -8, -5]}>
      <mesh geometry={SUN_GEO} material={SUN_MAT} />
      <Planet radius={0.2} orbitRadius={3.5} speed={0.8} color="#94a3b8" />
      <Planet radius={0.3} orbitRadius={5} speed={0.6} color="#fbbf24" />
      <Planet radius={0.3} orbitRadius={7} speed={0.5} color="#3b82f6"
        moons={[{ radius: 0.08, orbitRadius: 0.7, speed: 2.5, color: "#d1d5db" }]} />
      <Planet radius={0.22} orbitRadius={9} speed={0.4} color="#ef4444"
        moons={[
          { radius: 0.05, orbitRadius: 0.5, speed: 3.0, color: "#a8a29e" },
          { radius: 0.04, orbitRadius: 0.75, speed: 2.0, color: "#d6d3d1" },
        ]} />
      <Planet radius={0.5} orbitRadius={11} speed={0.25} color="#c084fc"
        moons={[
      ...
      <Planet radius={0.45} orbitRadius={14} speed={0.18} color="#f97316" hasRings
        moons={[{ radius: 0.07, orbitRadius: 1.6, speed: 1.5, color: "#fbbf24" }]} />
      <Planet radius={0.28} orbitRadius={17} speed={0.12} color="#06b6d4"
